...
export default function SubmitForm() {
  ...
  // Handler específico para o título
  ...
  const handleLocation = async () => {
    ...
    try {
      ...
      // Use our new specialized location helper
      ...
      if (locationResult.success && locationResult.position) {
        ...
        // Fallback to the improved hook approach if the helper fails
        console.log("Helper approach failed, trying the hook method");
        
        const hookSuccess = await geolocation.getFullLocation({
          enableHighAccuracy: true,
          timeout: 30000,
          maximumAge: 0
        });
        
        if (hookSuccess && geolocation.latitude && geolocation.longitude) {
          console.log("Successfully retrieved location with geolocation hook!");
          
          // Update form state with the location data from the hook
          setFormState(prev => ({
            ...prev,
  ...
  const renderStepContent = () => {
    switch (currentStep) {
      case 1:
        console.log('Rendering step 1 with title:', formState.title);
        return (
