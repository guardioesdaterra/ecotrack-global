    ...
    if (preferReducedMotion === true || (!isVisible && !isInViewport)) {
      return (
        <div
          ref={elementRef}
          className={cn(
            "absolute h-full w-full inset-0 flex items-center justify-center pointer-events-none opacity-50",
            className,
          )}
        >
          {/* Simplified static version */}
          <div className="absolute inset-0 bg-gradient-to-tr from-cyan-900/10 via-transparent to-purple-900/10 opacity-30" />
        </div>
      );
    }
