...
const MonitorDashboard = dynamic(() => import("@/app/monitor/page"), {
  ssr: false,
  loading: () => <LoadingIndicator />
})
...
export function Overlay() {
  const { overlayType, hideOverlay, isOverlayVisible } = useOverlay()
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          >
            ...
            <div className="absolute inset-0 pointer-events-none overflow-hidden">
              ...
              <div className="absolute top-0 bottom-0 left-0 w-[1px] bg-gradient-to-b from-transparent via-cyan-500/50 to-transparent"></div>
              <div className="absolute top-0 bottom-0 right-0 w-[1px] bg-gradient-to-b from-transparent via-purple-500/50 to-transparent"></div>
            </div>
            
            {/* Close button with enhanced cyberpunk style */}
            <button 
              onClick={hideOverlay}
              className="absolute top-4 right-4 z-30 p-2 rounded-full bg-black/80 hover:bg-gray-800/80 border border-cyan-900/50 shadow-[0_0_8px_rgba(6,182,212,0.2)] hover:shadow-[0_0_12px_rgba(6,182,212,0.4)] transition-all duration-300 group overflow-hidden"
            >
              {/* Glow effect on hover */}
              <div className="absolute inset-0 rounded-full bg-gradient-to-r from-cyan-900/0 to-cyan-900/0 group-hover:from-cyan-900/20 group-hover:to-cyan-900/30 transition-all duration-300"></div>
            ...
            <div className="h-full w-full overflow-y-auto overflow-x-hidden custom-scrollbar">
              {overlayType === "submit" && <SubmitForm />}
              {overlayType === "monitor" && <MonitorDashboard />}
            </div>
          </motion.div>
        </div>
      )}
