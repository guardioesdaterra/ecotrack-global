...
// Import the original type just for reference
    ...
    >
      <div 
        ...
        style={{
          ...
          boxShadow: `0 0 30px ${activityColor}50, 0 0 50px ${activityColor}20`,
      ...
      >
        ...
        >
          ...
          <div 
            ...
            style={{ 
              border: `1px solid ${activityColor}40`,
              boxShadow: `0 0 10px ${activityColor}30`
            }}
          >
          ...
          {photos.length > 1 && (
            <>
              ...
              <button 
                ...
                style={{ 
                  boxShadow: `0 0 15px ${activityColor}20`,
        ...
        {photos.length > 1 && (
          <div 
            ...
            style={{ 
              height: '70px', 
              borderTop: `1px solid ${activityColor}30`,
          ...
          >
            {photos.map((photo, index) => (
              <div 
                ...
                className="h-full cursor-pointer flex-shrink-0 transition-transform hover:scale-105"
                style={{ 
                  width: 'auto',
                  border: index === currentIndex ? `2px solid ${activityColor}` : '2px solid transparent',
                  boxShadow: index === currentIndex ? `0 0 10px ${activityColor}60` : 'none'
                }}
              >
                <img 
                  src={photo} 
                  ...
                  style={{ 
                    filter: index !== currentIndex ? 'grayscale(50%)' : 'none',
                    opacity: index !== currentIndex ? 0.7 : 1
                  }}
                />
              </div>
            ))}
...
function FlyToAnimation({ event }: { event: FlyToEvent | null }) {
  ...
  useEffect(() => {
    ...
    // Create a visual indicator for animation focus
    const focusElement = document.createElement('div');
    focusElement.className = 'map-focus-indicator';
    focusElement.style.cssText = `
      ...
      box-shadow: 0 0 20px ${event.color};
    ...
    // Perform the flyTo animation with enhanced options
    map.flyTo([event.lat, event.lng], event.zoom, {
      duration: 2.5,
      easeLinearity: 0.25,
    });
    
    // Position the focus indicator on the map
    const updateFocusPosition = () => {
      const point = map.latLngToContainerPoint([event.lat, event.lng]);
      focusElement.style.left = `${point.x}px`;
      focusElement.style.top = `${point.y}px`;
    };
    ...
    // Add wind effect lines
    if (windLinesRef.current) {
      const lines = windLinesRef.current.querySelectorAll('.wind-line');
  ...
  // Only render the wind effect during animation
    ...
    >
      ...
      {Array.from({ length: 20 }).map((_, i) => (
        <div 
          ...
          style={{
            ...
            height: '2px',
            background: `linear-gradient(90deg, transparent, ${event?.color || '#06b6d4'}80, transparent)`,
            transform: `rotate(${Math.random() * 360}deg)`,
            opacity: 0,
            animation: 'windLineAnimation 1.5s ease-in-out forwards'
          }}
        />
      ))}
      ...
      >
        <div 
          ...
          style={{
            background: `radial-gradient(circle, ${event?.color || '#06b6d4'}40 0%, transparent 70%)`,
...
function ActivityNode({ activity }: { activity: Activity }) {
  ...
  // Create the icon with a distinctive appearance to make it more visible
...
export default function MapClient({ activities: propActivities }: MapClientProps) {
    ...
    const fetchActivities = async () => {
      try {
        ...
        const activitiesData = data.map(item => ({
          ...
          direct_benefited: item.direct_benefited || 1000,
          indirect_benefited: item.indirect_benefited || 5000,
        }));
  ...
  return (
    <div className="relative w-full h-full">
      {/* Remove loading screen that caused flickering */}
